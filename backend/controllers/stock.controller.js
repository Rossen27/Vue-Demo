...
export const getStockInfo = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    if (!ex_ch) {
      return res.status(400).json({ message: 'ex_ch query parameter is required' });
    }

    const stockArray = ex_ch.split('|'); // 將股票代號分割成數組
    const stockList = stockArray.map(stock => stock.startsWith('tse_') || stock.startsWith('otc_') ? stock : `tse_${stock}.tw|otc_${stock}.tw`).join('|');
